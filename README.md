...
│   └── schema-trip.sql     行程域（运行时配置/POI 缓存/调用日志，行程表族追加于此）
├── wayfare-backend/
...
│       ├── controller/     REST 接口（内容域 + 社交域 + 诊断）
│       ├── entity|mapper/  17 张表的实体与 Mapper
...
- **测试**：单元测试覆盖脱敏规则（6 例）、重试与降级策略（9 例）、实体与表结构映射（4 例），共 19 例全绿。
