...
│       │   ├── trip/           编排管线：意图解析（Step 1）、候选检索（Step 2）、空间预排（Step 3）、错误码、阶段记录
...
- **测试**：119 个单元 / 集成测试全绿（另有 4 个真实调用大模型与地图的验收测试默认跳过，
...
| P3 | 七步 AI 行程编排管线（核心） | 🔨 进行中（Step 1 意图解析、Step 2 候选检索、Step 3 空间预排已完成） |
